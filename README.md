...
6. Run `python nbgrader2canvas.py` and enter the Canvas ID of the course and assignment (located in the URL of the assignment page). The script requires your Canvas token saved in a file called `token.txt`. The sciprt uploads the grade and autograded notebook to Canvas for each student. The file `upload_log.csv` records which autograded notebooks have been uploaded so that you can run the script again and it will skip over the students whose autograded notebook has not changed.
